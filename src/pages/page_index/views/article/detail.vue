...
<template>
    <section v-loading.fullscreen.lock="loading" element-loading-background="rgba(0,0,0,0.05)">
        ...
        <div class="auditfail-reason-box" v-if="this.dialogData.audit==2">
            审核未通过，原因：{{dialogData.reason}}
        </div>
        <el-form ref="dialogForm1" label-width="120px" :model="dialogData" :rules="dialogRule1" style="width:95%" @submit.native.prevent :disabled="readOnly">
            ...
            <el-form-item label="发布时间" prop="time" style="display:inline-block;width:50%;">
                <el-date-picker v-model="dialogData.time" type="date" value-format="timestamp" style="width:140px;">
                </el-date-picker>
            </el-form-item>
        </el-form>
    ...
    methods: {
        ...
        save(submitStatus) {
            ...
            this.$refs.dialogForm1.validate(valid => {
                if (valid) {
                    // 附件和内容至少填一个
                    if (this.$refs.xEditor.checkContentIsNotNull() === false && param.list.length == 0) {
                        this.$message({
                            message: '内容和附件不能同时为空！',
                            type: 'warning',
                            showClose: true,
                    ...
                    this.xAxios({
                        xJsonData: true,
                        url: BASE_PATH + `/article/${this.mode==1?"add":"edit"}.htmls`,
                        data: param
                    }).then((response) => {
                        const res = response.data;
                        // 数据格式化
