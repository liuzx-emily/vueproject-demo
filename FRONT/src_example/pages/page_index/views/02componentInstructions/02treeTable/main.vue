...
<template>
    <section>
        <el-card>
            <div slot="header">
                使用这个组件一定要传<code>columns</code>
            </div>
            <!-- card body -->
            <p>控制列：内含加号或减号，控制展开与收缩</p>
            <p>如果columns为0，那么会自动在最前面加一个序号列，并且把这个序号列作为"控制列"</p>
            <p>如果columns不为0，那么会自动取columns中的第一列作为"控制列"</p>
            <p>但是，columns为0时出现的序号列不好看。所以columns不要为0。</p>
        </el-card>
        <el-card>
            <div slot="header">
            ...
            <p>但因为这并不是我们想要的，所以别用children，用其他什么都行，我这里用的是child</p>
            <p>至于，还是用自己写的组件呢？</p>
        </el-card>
        <el-card>
            <div slot="header">
        ...
        <treeTable :data="data" :columns="columns" border>
            <el-table-column prop="timeLine" label="列可以直接写在这里"></el-table-column>
        </treeTable>
    </section>
</template>
<script>
