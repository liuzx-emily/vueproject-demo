...
<template>
    <section v-loading.lock="loading" element-loading-background="rgba(0,0,0,0.05)">
        <el-card>
            ...
            <p>效果：
                <el-radio-group v-model="fixBug">
                    <el-radio :label="false">原汁原味</el-radio>
                    <el-radio :label="true">处理bug</el-radio>
                </el-radio-group>
            </p>
        </el-card>
        <el-card>
            <div slot="header">怎么解决</div>
            <!-- card body -->
            <p>获取数据之后，先处理一下，把组数少的补齐。将补充的数据的<code>value</code>设置为<code>undefined</code></p>
        </el-card>
...
export default {
    ...
    methods: {
        ...
        getOption(data) {
            ...
            option.series = this._.map(data, item => {
