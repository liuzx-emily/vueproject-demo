	...
	<section v-loading="loading">
		<section class="abovelist-search-and-btns">
			<section class="btns-box">
				<x-button v-permission="'org:add'" @click="openDial_main(1)" icon="plus">新增</x-button>
			</section>
		</section>
		<section>
			<x-table :refresh="srefresh" :enablePaging="false" ref="table" row-key="id" default-expand-all>
				...
				<el-table-column label="操作">
					<template v-slot="scope">
						<x-button v-permission="'org:edit'" @click="openDial_main(2,scope.row)" title="编辑" icon="pencil"></x-button>
						<x-button v-permission="'org:detail'" @click="openDial_main(3,scope.row)" title="查看" icon="eye"></x-button>
						<x-button v-permission="'org:delete'" @click="openConfirm_delete(scope.row)" title="删除" icon="trash" color="red"></x-button>
					</template>
				</el-table-column>
			</x-table>
		</section>
