	...
	<section v-loading.fullscreen.lock="loading">
		<section class="abovelist-search-and-btns">
			<section class="btns-box">
				<x-button v-permission:dept="1" @click="openDial_main(1)">新增</x-button>
			</section>
		</section>
		<gc-treetable :data="treeTableData" :controlColumn="treeTableProps.controlColumn" :expandAll="treeTableProps.expandAll">
			...
			<el-table-column label="操作" width="250px">
				<template slot-scope="scope">
					<x-button v-permission:dept="2" @click="openDial_main(2,scope.row)">编辑</x-button>
					<x-button v-permission:dept="3" @click="openDial_main(3,scope.row)">查看</x-button>
					<x-button v-permission:dept="4" @click="openConfirm_delete(1,scope.row)" color="red">删除</x-button>
				</template>
			</el-table-column>
		</gc-treetable>
		<!-- 弹窗：主 -->
