...
const xRules = {
  ...
  western: {
    ...
    check(val) {
      return (!xRules.required.check(val)) || /^[a-zA-Z0-9_]+$/.test(val);
    }
  },
  // 整数：负数+0+正数，只能是整数
  integer: {
    message: "必须为整数！",
    check(val) {
      return (!xRules.required.check(val)) || /^-?\d+$/.test(val);
    }
  },
  // 小数：负数+0+正数，小数整数都行
  decimal: {
    message: "必须为数字！",
    check(val) {
      return (!xRules.required.check(val)) || /^(-?\d+)(\.\d+)?$/.test(val);
    }
  },
  // 电话：固话+手机
  phone: {
    ...
    check(val) {
      return (!xRules.required.check(val)) || xRules.cellphone.check(val) || xRules.fixedphone.check(val);
    }
  },
  // 手机：以1开头，共11位
  cellphone: {
    message: "必须为11位手机号码！",
    check(val) {
      return (!xRules.required.check(val)) || /^1\d{10}$/.test(val);
    }
  },
  // 固话
  fixedphone: {
    message: "必须为座机！",
    check(val) {
      return (!xRules.required.check(val)) || /^0\d{2,3}-?\d{7,8}$/.test(val);
    }
  },
  // 邮箱
  email: {
    message: "必须为合法的邮箱格式！",
    check(val) {
      return (!xRules.required.check(val)) || /^(\w-*\.*)+@(\w-?)+(\.\w{2,})+$/.test(val);
    }
  }
};
// 校验函数
...
const xElementRules = { // 
  ...
  phone: (rule, value, callback) => {
    if (xRules.phone.check(value)) {
      return callback();
    } else {
      callback(new Error(xRules.phone.message));
    }
  },
};
